...
if [[ -d "${APP_BUNDLE}" ]]; then
  OLD_DESIGNATED_REQUIREMENT=$(codesign -dr - "${APP_BUNDLE}" 2>&1 | tail -1 || true)
fi
...
NEW_DESIGNATED_REQUIREMENT=$(codesign -dr - "${APP_BUNDLE}" 2>&1 | tail -1 || true)
if [[ "${OLD_DESIGNATED_REQUIREMENT}" == *"designated => cdhash"* \
   && "${NEW_DESIGNATED_REQUIREMENT}" != *"designated => cdhash"* ]]; then
  log "==> Resetting the stale Accessibility record from the previous ad-hoc signature"
  if ! tccutil reset Accessibility "${BUNDLE_ID}"; then
    log "WARN: Could not reset Accessibility automatically. Remove Lumen from System Settings > Privacy & Security > Accessibility, then add it again."
  fi
  log "NOTE: Grant Accessibility once more for the new stable identity; future rebuilds will retain it."
fi
